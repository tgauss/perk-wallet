...
- **README Overhaul**: Complete documentation rewrite with quick start guide
  - Identity model explanation with examples and Mermaid ER diagram
  - Routes overview with public install routes and admin redirect patterns
  - Template Studio features and merge tags reference
  - Diagnostics API documentation with expected responses
  - Deploy to Vercel step-by-step instructions
  - Concepts at a glance checklist for new engineers

- **Comprehensive Documentation Suite** (`/docs/`)
  - `ARCHITECTURE.md`: System design, request flow patterns, security architecture
  - `ROUTES.md`: Complete API reference with examples and error codes
  - `IDENTITY.md`: Composite key architecture and dual addressing guide
  - `TEMPLATE_STUDIO.md`: Template system, merge tags, and preview functionality
  - `OPERATIONS.md`: Admin tasks, troubleshooting, and operational procedures
  - `ENV.md`: Complete environment variables reference with security best practices

- **Enhanced Environment Configuration**
  - Refreshed `.env.example` with organized variable groups
  - Detailed setup notes for local development and Vercel production
  - Security guidelines and secret rotation recommendations
  - Variable-by-variable documentation with usage examples

### Changed

- **STATE.md**: Updated to reflect v0.6.0 identity model lock status
  - Composite key architecture verification
  - Diagnostics and install routes confirmed green
  - Template publication status for program 44
  - Environment requirements clarification

### Documentation

- **Self-Explanatory Repository**: New engineers can now onboard independently
- **Complete API Reference**: All routes documented with request/response examples
- **Operational Procedures**: Step-by-step guides for common admin tasks
- **Architecture Overview**: High-level system design with security patterns
- **Environment Security**: Comprehensive security best practices

### Technical Details

- **No Runtime Changes**: Documentation-only release
- **Markdown Formatting**: Consistent styling and navigation
- **Cross-References**: Links between related documentation sections
- **Code Examples**: Real-world usage patterns and troubleshooting guides

## [v0.6.0] – 2025-08-15

### Added

- **Identity Model Lock**: Composite key architecture finalized and verified
  - Program resolver utility with dual addressing support
  - Admin short routes: `/admin/p/{perkProgramId}` → UUID-based admin routes
  - Complete migration from `perk_uuid` to `(program_id, perk_participant_id)`
  - QR grammar: `programId.participantId.passKind[.resourceType.resourceId]`

- **End-to-End Verification**: Full identity model validation
  - Diagnostics API confirms participant existence and template readiness
  - Install routes verified: `/w/44/246785` returning 200 with proper response
  - Admin redirect functional: proper 307 redirect to UUID-based routes
  - No UUID exposure in public APIs

### Changed

- **Database Schema Migration**: Complete composite key implementation
  - `participants` table: Primary key `(program_id, perk_participant_id)`
  - `passes` table: Foreign key references composite participant key
  - Removed all `perk_uuid` columns and references

- **Route Parameter Updates**: All routes use composite identity
  - Public routes: Use Perk Program IDs (`/w/44/246785`)
  - Admin routes: Use internal UUIDs with resolver bridging
  - API routes: Updated to support composite key lookups

### Fixed

- **Diagnostics API**: Participant detection using `maybeSingle()` instead of `single()`
- **Template Preview**: Duplicate property removal in participant data
- **Route Compatibility**: Next.js 15 async params handling
