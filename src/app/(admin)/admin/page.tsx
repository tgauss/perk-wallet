...
// Mock KPIs - in real app this would come from database
const getSuperAdminKPIs = () => ({
  totalPrograms: 5,
...
const getProgramKPIs = (programId: string) => ({
  participants: 2450,
  activePasses: 1820,
  passesIssued24h: 89,
  failedJobs: 3,
...
export default async function AdminDashboard() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <TooltipProvider>
        {canViewAll ? <SuperAdminDashboard /> : <ProgramDashboard programId={identity.programId} />}
      </TooltipProvider>
    </div>
  )
}
...
function ProgramDashboard({ programId }: { programId?: string }) {
  if (!programId) {
    return (
      <Card>
  ...
  const kpis = getProgramKPIs(programId)
